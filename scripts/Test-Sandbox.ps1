...
Param(
  [Parameter(Mandatory, HelpMessage = "The path to the nuspec/nupkg file to test.")]
  [String] $Nuspec
)

if (-not (Test-Path -Path $Nuspec -PathType Leaf)) {
  throw 'The nuspec/nupkg file does not exist.'
}
...
Copy-Item -Path $Nuspec -Destination $tempFolder

# Create Bootstrap script

$NuspecFileName = Split-Path $Nuspec -Leaf

$bootstrapPs1Content = @"
Write-Host 'Installing Chocolatey'
Set-ExecutionPolicy Bypass -Scope Process -Force; [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072; iex ((New-Object System.Net.WebClient).DownloadString('https://chocolatey.org/install.ps1'))
Write-Host 'Trying to install the specified package'
Set-PSDebug -Trace 1
choco install '$NuspecFileName'
"@
...
Write-Host 'Starting Windows Sandbox and trying to install the nuspec/nupkg file.'
