...
# It returns a hash map with a 'bit32' and 'bit64'. These values are not empty if the
...
function thisJreInstalled($version) {
    ...
    return @{
        'bit32' = $productSearch | Where-Object {$_.Name -match $nameRegex32 -and $_.Version -match $versionRegex}
        'bit64' = $productSearch | Where-Object {$_.Name -match $nameRegex64 -and $_.Version -match $versionRegex}
    }
}
