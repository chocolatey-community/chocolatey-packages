...
if ($alreadyInstalled -and ($env:ChocolateyForce -ne $true)) {
  ...
  $packageArgs = @{
    ...
    ChecksumType = 'sha512'
    Url = "https://download.mozilla.org/?product=thunderbird-60.7.0-SSL&os=win&lang=${locale}"
  ...
  if (!(Get-32bitOnlyInstalled($softwareName)) -and (Get-OSArchitectureWidth 64)) {
    ...
    $packageArgs.Url64 = "https://download.mozilla.org/?product=thunderbird-60.7.0-SSL&os=win64&lang=${locale}"
  }
