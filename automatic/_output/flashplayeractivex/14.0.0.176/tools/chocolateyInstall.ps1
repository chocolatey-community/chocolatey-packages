...
$version = '14.0.0.176' # This is the real version of Flash Player ActiveX (is not always the same as the package version)
...
try {
    ...
    if ($allRight) {
        Install-ChocolateyPackage $packageName 'msi' '/quiet /norestart REMOVE_PREVIOUS=YES' 'http://download.macromedia.com/get/flashplayer/current/licensing/win/install_flash_player_14_active_x.msi'
    }
