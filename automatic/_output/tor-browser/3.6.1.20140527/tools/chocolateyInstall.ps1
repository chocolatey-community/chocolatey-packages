...
try {
    ...
    if ($env:ChocolateyBinRoot) {
        ...
        Write-Output 'Warning: no $env:ChocolateyBinRoot set. tor-browser will be extracted to $env:SystemDrive\tools\tor-browser'
    }
...
"@
    ...
    $url = "https://www.torproject.org/dist/torbrowser/3.6.1/torbrowser-install-3.6.1_${langcode}.exe"
