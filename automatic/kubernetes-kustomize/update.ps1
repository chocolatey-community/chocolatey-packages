...
$releases = "$domain/kubernetes-sigs/kustomize/releases/latest"

function global:au_BeforeUpdate {
  Get-RemoteFiles -Purge -NoSuffix -FileNameBase "kustomize"
}
...
function global:au_GetLatest {
  ...
  return @{
    Version     = $version
    URL64       = $domain + $url
    ReleaseURL  = "$domain/kubernetes-sigs/kustomize/releases/tag/v${version}"
  }
}
