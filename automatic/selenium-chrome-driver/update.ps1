...
function global:au_AfterUpdate {
  Update-Metadata -key 'releaseNotes' -value (Invoke-WebRequest -Uri $Latest.UrlReleaseNotes -UseBasicParsing -ContentType "text/plain" | % Content)
}

function global:au_GetLatest {
  $release_page = Invoke-WebRequest -Uri $releases -UseBasicParsing

  $urls = $release_page.Links | ? href -match "path=(\d+\.){3}\d+\/$" | select -expand href
